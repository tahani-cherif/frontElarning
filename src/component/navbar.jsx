  ...
  return (
    <div style={{backgroundColor:"#144272"}}>
      <div className="flex space-x-4  h-[74px] shadow-lg text-center justify-between items-center px-4">
        ...
        <Link to={user?.role==="admin" ?"/admin" :user?.role==="formateur" ? "/formateur" : user?.role==="user"? "/user" :"/"} className='flex flex-row gap-3'>
         <Avatar alt="Remy Sharp" src='/logo-colored.jpeg'  />
          <h2 className="text-3xl font-bold" style={{color:"white"}}>RAPYD LEARN</h2>
          </Link>
       ...
       <Link to="/contact"> <p className='cursor-pointer'>Contact</p></Link>
        ...
        <div className='hidden md:flex pr-4 space-x-4 justify-end' >
          {localStorage.getItem('tocken')!=null ?<Link to={'/editprofile'}>
            <Avatar alt="Remy Sharp" src={user?.image  ? "http://localhost:8000/"+user?.image : "./images.png" }  /> </Link>:<Link to="/login"><button className='border border-black h-10 text-sm font-bold w-29 ' style={{borderRadius: "7px",padding: "10px",backgroundColor:"#2C74B3",    borderLeftColor: "#2C74B3",
